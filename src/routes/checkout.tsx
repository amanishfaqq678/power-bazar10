...
function CheckoutPage() {
  ...
  if (items.length === 0) return <SiteLayout><div className="container-pb py-20"><h1 className="text-2xl font-extrabold">Your cart is empty</h1><p className="mt-2 text-muted-foreground">Add products before continuing to checkout.</p><Button asChild className="mt-5 rounded-full"><Link to="/products" search={{ q: undefined, category: undefined }}>Browse products</Link></Button></div></SiteLayout>;

  return <SiteLayout>
    <PageHeader eyebrow="Secure checkout" title="Complete your order" description="Cash on delivery is currently available for your Power Bazar order." />
    <section className="container-pb py-10">
      <form onSubmit={submit} className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
          <div className="grid gap-5 sm:grid-cols-2">
            {([["customer_name", "Full name", true], ["phone", "Mobile / WhatsApp number", true], ["email", "Email (optional)", false], ["city", "City", true]] as const).map(([key, label, required]) => <div key={key}><Label htmlFor={key}>{label}{required ? " *" : ""}</Label><Input id={key} required={required} type={key === "email" ? "email" : key === "phone" ? "tel" : "text"} value={form[key]} onChange={(event) => update(key, event.target.value)} className="mt-2 h-11 rounded-lg" /></div>)}
          </div>
          ...
          <Button type="submit" disabled={busy} className="mt-6 h-12 w-full rounded-full font-extrabold">{busy ? "Placing order…" : "Place Order"}</Button>
        </div>
        <aside className="h-fit rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)] lg:sticky lg:top-24"><h2 className="text-lg font-extrabold">Order summary</h2>{items.map((item) => <div key={item.productId} className="mt-4 flex justify-between gap-3 text-sm"><span>{item.productName} × {item.quantity}</span><span className="font-bold">PKR {((item.unitPrice ?? 0) * item.quantity).toLocaleString()}</span></div>)}<div className="mt-5 flex justify-between border-t border-border pt-4 text-lg font-extrabold"><span>Total</span><span>PKR {subtotal.toLocaleString()}</span></div></aside>
      </form>
    </section>
  </SiteLayout>;
}
