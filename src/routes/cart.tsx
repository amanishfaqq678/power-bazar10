  ...
  return (
    <SiteLayout>
      ...
      <section className="container-pb py-10">
        {items.length === 0 ? (
          <div className="motion-fade-up mx-auto max-w-xl rounded-2xl border border-border bg-card p-10 text-center shadow-[var(--shadow-card)]">
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
            <div className="space-y-3">
              {items.map((item) => (
                <article key={item.productId} className="motion-fade-in flex gap-4 rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
                  <img src={categoryImage(item.categorySlug, item.imageUrl)} alt={item.productName} className="size-20 rounded-lg bg-surface object-cover" />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <span className="w-8 text-center font-bold tabular-nums" aria-label={`Quantity ${item.quantity}`}>{item.quantity}</span>
