-- Retail order system. Prices and stock are resolved inside this function.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM jsonb_array_elements(order_input->'items') AS requested_item
    GROUP BY requested_item->>'product_id'
    HAVING count(*) > 1
  ) THEN
    RAISE EXCEPTION 'Each product may only appear once in an order';
  END IF;

  FOR item IN SELECT value FROM jsonb_array_elements(order_input->'items')
  LOOP
    ...
    IF NOT FOUND OR NOT product_row.price_available OR product_row.price IS NULL THEN
      RAISE EXCEPTION 'Product is unavailable for purchase';
    END IF;
    IF product_row.availability = 'out_of_stock' OR product_row.stock_quantity < item_quantity THEN
    ...
    UPDATE public.products
      SET stock_quantity = stock_quantity - item_quantity,
          availability = CASE WHEN stock_quantity - item_quantity <= 0 THEN 'out_of_stock'
                               WHEN stock_quantity - item_quantity <= 10 THEN 'low_stock'
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Inventory record missing for %', product_row.name;
    END IF;
  END LOOP;

  INSERT INTO public.orders (
    order_number, customer_name, phone, email, address, city, subtotal, total,
    payment_method, payment_status, order_status, customer_note
  ) VALUES (
    'PB-' || to_char(clock_timestamp(), 'YYYYMMDDHH24MISS') || '-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 6)),
    trim(order_input->>'customer_name'), trim(order_input->>'phone'), nullif(trim(order_input->>'email'), ''),
    trim(order_input->>'address'), trim(order_input->>'city'), calculated_subtotal, calculated_subtotal,
    'cod', 'pending', 'new', nullif(trim(order_input->>'customer_note'), '')
  ...
  LOOP
    ...
    VALUES (new_order.id, product_row.id, product_row.name, product_row.price, item_quantity, item_total)
    RETURNING jsonb_build_object('id', id, 'product_id', product_id, 'product_name_snapshot', product_name_snapshot,
      'unit_price', unit_price, 'quantity', quantity, 'line_total', line_total) INTO item;
    item_output := item_output || jsonb_build_array(item);
  END LOOP;

  RETURN jsonb_build_object('id', new_order.id, 'order_number', new_order.order_number,
    'customer_name', new_order.customer_name, 'phone', new_order.phone, 'email', new_order.email,
    'address', new_order.address, 'city', new_order.city, 'subtotal', new_order.subtotal,
    'delivery_fee', new_order.delivery_fee, 'total', new_order.total, 'payment_method', new_order.payment_method,
    'payment_status', new_order.payment_status, 'order_status', new_order.order_status,
    'customer_note', new_order.customer_note, 'created_at', new_order.created_at, 'items', item_output);
END;
$$;
...
BEGIN
  IF NEW.id <> OLD.id
    ...
    RAISE EXCEPTION 'Order identity, customer, payment method and totals are immutable';
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER orders_financial_fields_immutable
